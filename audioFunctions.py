...
def frame_multi_F0_match(fft_x, fftSize, rate, freqMin, freqMax, candidates):
	isMatched = np.ones(len(candidates)) * False
	X = fft(fft_x)
	...
	for i in range(len(candidates)):
		...
		fc_h_mag = np.array([])
		fc_h_index = np.array([])
		# Get fc_h info w/o removing shared harmonics
		...
		# print '=> fc_h_mag = \n', fc_h_mag
		# print '=> fc_h_index = \n', fc_h_index
		print np.sum(fc_h_mag)
		# Check the threshold to determine if a candidate is matched
		if (np.sum(fc_h_mag) > maxFrequencyMagnitude*1.3) & (fc_h_mag[0] > 0) & ((fc_h_mag[fc_h_mag==0]).size < 4):
			...
			for index in range(H):
				# print fc_h_index[index], fc_h_mag[index], Xabs[int(fc_h_index[index])]
			...
			print 'updated maxFrequencyMagnitude = ', maxFrequencyMagnitude, '\n'
		else:
			print '**** F0:\t', freqBase[candidates[i]], ' NOT Found\n'
		

	falseRemain = isMatched[isMatched == False]
	if falseRemain.size:
		return False
	else:
		return True